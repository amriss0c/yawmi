...
class CalendarScreen extends StatelessWidget {
            ...
            return Directionality(
              ...
              child: Scaffold(
                ...
                body: GestureDetector(
                  ...
                  child: isWide
                      ? _buildWideLayout(context, provider, width)
  ...
  PreferredSizeWidget _buildAppBar(BuildContext context, TaskProvider provider, bool isWide) {
    ...
    return AppBar(
      ...
      title: Row(
        children: [
          Column(
            ...
            children: [
              Text(HijriHelper.getGregorianMonthName(focusedDate, true),
                  style: const TextStyle(color: Colors.white, fontSize: 18)),
              Text(HijriHelper.getHijriMonthRange(focusedDate, true),
      ...
      actions: [
        IconButton(icon: const Icon(Icons.today, color: Colors.white),
            onPressed: () => provider.goToToday(), tooltip: 'اليوم'),
      ...
      Expanded(
        child: GridView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return DayCell(
              date: date, task: provider.getTask(date),
  ...
  Widget _buildWideLayout(BuildContext context, TaskProvider provider, double screenWidth) {
    final focusedDate = provider.focusedMonth;
    final daysInMonth = DateUtils.getDaysInMonth(focusedDate.year, focusedDate.month);
    final firstDay = DateTime(focusedDate.year, focusedDate.month, 1);
    int weekdayOffset = firstDay.weekday;
    ...
    final weekDays = provider.startOnSaturday
        ? ['س', 'ح', 'ن', 'ث', 'ر', 'خ', 'ج']
        : ['ن', 'ث', 'ر', 'خ', 'ج', 'س', 'ح'];
    final cardHeight = screenWidth > 800 ? 68.0 : 62.0;
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Expanded(
          ...
          child: Column(children: [
            ...
            _buildBottomCards(context, provider, cardHeight),
          ]),
        ),
        VerticalDivider(width: 1, color: const Color(0xFF1A6B4A).withOpacity(0.2)),
        Expanded(flex: 35, child: _buildDetailPanel(context, provider)),
      ],
    );
  }

  Widget _buildBottomCards(BuildContext context, TaskProvider provider, double cardHeight) {
    ...
    final day = today.day;
    String motivEmoji; String motivText;
    if (progress == 1.0) {
      motivEmoji = '🌟';
      final msgs = ['ما شاء الله — أكملت شهرك كاملاً، هذا ليس سهلاً','شهر كامل بلا انقطاع — أنت تصنع عادة لا تُكسر','اكتمل الشهر على خير — بارك الله في وقتك وجهدك'];
      motivText = msgs[day % msgs.length];
    } else if (progress >= 0.75) {
      motivEmoji = '💪';
      final msgs = ['أنت في المستوى الذي يحلم به كثيرون — لا تتوقف الآن','ثلاثة أرباع الشهر منجزة — النهاية قريبة، أكملها','الاتساق هو أصعب ما يملكه الناس — وأنت تملكه'];
      motivText = msgs[day % msgs.length];
    } else if (progress >= 0.5) {
      motivEmoji = '🎯';
      final msgs = ['تجاوزت النصف — من وصل هنا لا يتوقف قبل النهاية','أنت في منتصف الطريق، والنصف الثاني أسهل دائماً','نصف الشهر خلفك — ركّز على ما أمامك'];
      motivText = msgs[day % msgs.length];
    } else if (progress >= 0.25) {
      motivEmoji = '🌱';
      final msgs = ['البداية الحقيقية صعبة — لكنك تجاوزتها بالفعل','كل يوم تُسجَّل فيه خطوة هو يوم لم يضع','ربع الشهر منجز — الزخم بدأ، لا تكسره'];
      motivText = msgs[day % msgs.length];
    } else if (progress > 0) {
      motivEmoji = '🔥';
      final msgs = ['البداية أصعب جزء — وقد بدأت','كل بطل كان يوماً مبتدئاً — أنت في الطريق الصحيح','أول خطوة تساوي نصف الرحلة'];
      motivText = msgs[day % msgs.length];
    } else {
      motivEmoji = '📅';
      final msgs = ['اليوم هو أفضل وقت للبداية — غداً سيكون أصعب','الشهر أمامك كاملاً — ابدأ ولو بيوم واحد','لا توجد لحظة مثالية للبدء — فقط ابدأ الآن'];
      motivText = msgs[day % msgs.length];
    }
    ...
    return Padding(
      ...
      child: Column(children: [
        Container(
          ...
          decoration: BoxDecoration(
            gradient: const LinearGradient(colors: [Color(0xFF1A6B4A), Color(0xFF2E9E6E)],
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              Text(hadithLabel, style: const TextStyle(color: Colors.white70, fontSize: 10, fontWeight: FontWeight.w600)),
              Text('يتغير يومياً', style: TextStyle(color: Colors.white.withOpacity(0.35), fontSize: 9)),
            ]),
            const SizedBox(height: 3),
            Text(hadith['text']!, style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.w500, height: 1.3), maxLines: 1, overflow: TextOverflow.ellipsis),
            const SizedBox(height: 2),
            Text('— ${hadith['source']}', style: TextStyle(color: Colors.white.withOpacity(0.5), fontSize: 9)),
          ]),
        ),
        ...
        Container(
          ...
          padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
          child: Row(children: [
            Text(motivEmoji, style: const TextStyle(fontSize: 22)),
            const SizedBox(width: 10),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
              Text(motivText, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: Color(0xFFE65100), height: 1.3), maxLines: 2, overflow: TextOverflow.ellipsis),
              const SizedBox(height: 2),
              Text('$pct٪ إنجاز هذا الشهر', style: TextStyle(fontSize: 9, color: const Color(0xFFBF360C).withOpacity(0.8))),
            ])),
          ]),
        ),
        const SizedBox(height: 4),
        Container(
          ...
          padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
          child: provider.quoteLoading
              ? const Center(child: SizedBox(width: 18, height: 18,
                  child: CircularProgressIndicator(strokeWidth: 2, color: Color(0xFF6A1B9A))))
              : provider.quoteArabic.isEmpty
                  ? Center(child: Text(
                      provider.quoteError.isNotEmpty ? provider.quoteError :
                      provider.quoteConfigured ? 'جارٍ تحميل الاقتباس...' : 'أضف مفتاح Gemini API في الإعدادات',
                      style: const TextStyle(fontSize: 10, color: Color(0xFF6A1B9A)), textAlign: TextAlign.center))
                  : Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
                      Row(children: [
                        const Text('💬', style: TextStyle(fontSize: 10)),
                        const SizedBox(width: 4),
                        const Text('اقتباس اليوم', style: TextStyle(fontSize: 9, color: Color(0xFF6A1B9A), fontWeight: FontWeight.w600)),
                        const Spacer(),
                        Flexible(child: Text(provider.quoteAuthor,
                            style: const TextStyle(fontSize: 9, color: Color(0xFF6A1B9A)), overflow: TextOverflow.ellipsis)),
                      ]),
                      const SizedBox(height: 3),
                      Text(provider.quoteArabic,
                          style: const TextStyle(fontSize: 11, color: Color(0xFF4A148C), fontWeight: FontWeight.w500, height: 1.3),
  ...
  Widget _buildProgressBar(TaskProvider provider) {
    ...
    return Padding(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          Text('إنجاز الشهر', style: TextStyle(fontSize: 11, color: Colors.grey.shade600)),
  ...
  Widget _buildTaskBox(BuildContext context, TaskProvider provider) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [BoxShadow(color: isDark ? Colors.black45 : Colors.black12, blurRadius: 8, offset: const Offset(0, 3))],
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
          ...
          if (task != null) Icon(task.status == 1 ? Icons.check_circle : Icons.pending,
              color: task.status == 1 ? Colors.green : Colors.orange, size: 20),
        ]),
        const Divider(height: 10),
        Text(
          task != null && task.taskText.trim().isNotEmpty ? task.taskText : 'لا توجد مهمة مسجلة لهذا اليوم',
          style: TextStyle(fontSize: 15,
              color: task != null && task.taskText.trim().isNotEmpty ? null : Colors.grey,
              fontStyle: task != null && task.taskText.trim().isNotEmpty ? FontStyle.normal : FontStyle.italic),
        ),
        const SizedBox(height: 4),
        Row(children: [
          TextButton.icon(
            ...
            style: TextButton.styleFrom(foregroundColor: const Color(0xFF1A6B4A),
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Icon(task?.status == 1 ? Icons.check_circle : task != null ? Icons.pending : Icons.radio_button_unchecked,
                  color: task?.status == 1 ? Colors.green : task != null ? Colors.orange : Colors.grey, size: 18),
              const SizedBox(width: 8),
              Text(task?.status == 1 ? 'منجزة' : task != null ? 'غير منجزة' : 'لا توجد مهمة',
                  style: TextStyle(color: task?.status == 1 ? Colors.green : task != null ? Colors.orange : Colors.grey,
                      fontWeight: FontWeight.bold, fontSize: 13)),
            ]),
            const SizedBox(height: 10),
            Text(task != null && task.taskText.trim().isNotEmpty ? task.taskText : 'لا توجد مهمة مسجلة',
                style: TextStyle(fontSize: 15,
                    color: task != null && task.taskText.trim().isNotEmpty ? null : Colors.grey,
                    fontStyle: task != null && task.taskText.trim().isNotEmpty ? FontStyle.normal : FontStyle.italic)),
          ]),
        ...
        Row(children: [
          Expanded(child: ElevatedButton.icon(
            icon: const Icon(Icons.edit, size: 15), label: const Text('تعديل'),
        ...
        const Text('ملخص الشهر', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14, color: Color(0xFF1A6B4A))),
        const SizedBox(height: 8),
        _buildProgressBar(provider),
        const SizedBox(height: 8),
        ...
        if (provider.quoteArabic.isNotEmpty) ...[
          const SizedBox(height: 8),
          const Divider(),
          const SizedBox(height: 8),
          Container(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                ...
                const SizedBox(width: 4),
                const Text('اقتباس اليوم', style: TextStyle(fontSize: 10, color: Color(0xFF6A1B9A), fontWeight: FontWeight.w600)),
                const Spacer(),
                Flexible(child: Text(provider.quoteAuthor,
                    style: const TextStyle(fontSize: 10, color: Color(0xFF6A1B9A)), overflow: TextOverflow.ellipsis)),
              ]),
              const SizedBox(height: 6),
              Text(provider.quoteArabic,
                  style: const TextStyle(fontSize: 12, color: Color(0xFF4A148C), fontWeight: FontWeight.w500, height: 1.5)),
            ]),
          ),
        ],
      ]),
  ...
  void _quickToggle(BuildContext context, TaskProvider provider, DateTime date) {
    ...
    if (task == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('لا توجد مهمة لتحديثها'), duration: Duration(seconds: 1)));
      return;
    }
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('تم التحديث: $newStatus'), duration: const Duration(seconds: 1)));
  }
